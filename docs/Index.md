...
* **SDL_RWFromConstMem**(const void* _mem_, int _size_): this prepares a read-only buffer memory buffer for use with SDL_RWops. _mem_ pointer to a read-only buffer, _size_ the buffer size in bytes.
